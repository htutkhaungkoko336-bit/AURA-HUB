...
const renderFirstPlayerOnly = (players) => {
    if (!players || players.length === 0) return "👤 Player မရှိပါ";
    const p = players[0]; // ပထမဆုံးတစ်ယောက်ကိုပဲ ယူမယ်
    return `👤 ${p.name} (ID: ${p.id})`;
};        
        // ပထမဆုံး Player (Index 0) ရဲ့ နာမည်နဲ့ ID ကိုပဲ ထုတ်ပြမယ့် Logic
const renderFirstPlayerOnly = (players) => {
    if (!players || players.length === 0) return "👤 Player မရှိပါ";
    const p = players[0]; // ပထမဆုံးတစ်ယောက်ကိုပဲ ယူမယ်
    return `👤 ${p.name} (ID: ${p.id})`;
...
// --- Bot Message ထဲမှာ ပြောင်းသုံးမယ့်ပုံစံ ---
const msg = `<b>🔍 MATCH DETAILS</b>\n\n🕒 <b>Time:</b> ${matchTime}\n💰 <b>Fee:</b> ${matchData.fee || 0}\n━━━━━━━━━━━━━━\n<b>🏆 TEAM A: ${matchData.teamA}</b>\n📞 Ph: ${dataA.kpayPhone}\n${renderFirstPlayerOnly(dataA.players)}\n\n<b>🏆 TEAM B: ${matchData.teamB}</b>\n📞 Ph: ${dataB.kpayPhone}\n${renderFirstPlayerOnly(dataB.players)}\n━━━━━━━━━━━━━━\n🎲 <b>First Pick:</b> ${matchData.firstPickWinner || 'N/A'}\n\n💡 * 5vs5 ဖြစ်စေ 1vs1 ဖြစ်စေ Fee ကြေးအလိုက် Bo(ပွဲအရေအတွက်) ဆော့ကစားရမည်။ 25000Ks နှင့် 50000 Ks room သည် Bo 3 ဆော့ကစားရမည်ဖြစ်ပြီး ကျန်သည့် room များမှာ Bo1 ဆော့ကစားရမည်။ID ချင်းဖလှယ်ကာ fri add ရမည်။ 5vs5 ကစားမည့်သူများသည် Custom Draft Pick တွင် ဆော့ကစားရမည်။ First pick သည် spin wheel တွင်တခါတည်းဆုံးဖြတ်ပြီးသားဖြစ်သည်။ 1vs1 ဆော့ကစားမည့်သူသည် vs A.I.mode အတွင်းမှ 1vs1 room တွင်ဆော့ကစားရမည်။id ချင်းဖလှယ်ကာ chat box မှဖြစ်စေ ph ဆက်၍ဖြစ်စေ ဆော့ကစားမည့်hero ကို သဘောတူရွေးချယ်နိုင်သည်။ same hero သာမက radom hero များပါ ၂ဦးသဘောတူကာရွေးချယ်နိုင်သည်။ အနိုင်ရရှိသည့် Team သည်  Result Screenshot ကို ပို့ပေးရမည်။ Bo 3 ဆော့ကစားသူများသည် 2:1ဖြစ်လျှင် screenshot 3 ခုလုံးပို့ပေးရမည်။ အနိုင်ရရှိသည့်တိုင် screenshot မပို့ပေးနိုင်လျှင် ငွေလွှဲပေးမည်မဟုတ်ပါ။ အငြင်းပွားဖွယ်ရာ အကြောင်းကိစ္စတစ်စုံတစ်ရာပေါ်လာပါက Admin ၏ ဆုံးဖြတ်ချက်သာ အတည်ဖြစ်သည်။ကျေးဇူးတင်ပါသည်။*`;        ctx.reply(msg, { parse_mode: 'HTML' });
