...
bot.action(/confirm_(.+)/, async (ctx) => {
    ...
    // ၁။ Pending Photo ထဲက Data ကို အရင်ယူမယ်
    const doc = await db.collection("pending_photos").doc(docId).get();
    ...
    try {
        ...
        await db.collection("registrations").doc(matchData.teamA_LeaderId).update({ status: "finished" });
        await db.collection("registrations").doc(matchData.teamB_LeaderId).update({ status: "finished" });
        ...
        await ctx.telegram.sendMessage(userId, "🎉 ဂုဏ်ယူပါသည်။ ပွဲစဉ်ရလဒ်ကို အတည်ပြုပြီးပါပြီ။ ကျေးဇူးပြု၍ Winner အတွက် ငွေလွှဲပြေစာ (SS) ကို Bot ထံသို့ ပို့ပေးပါ။");
